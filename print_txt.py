...
def print_txt(distable_linkdicts, all_xyz_rsteps, step, results_path):
    '''
    Record simulation results to .txt files.
    ...
    mylinks = links[step]
    try:
        os.remove(results_path + '/links.txt')
    except:
        pass
    try:
        os.remove(results_path + '/distance.txt')
    except:
        pass
    try:
        os.remove(results_path + '/links_distance.txt')
    except:
        pass
    try:
        os.remove(results_path + '/xyz.txt')
    except:
        pass
    ...
    file = open(results_path + '/distance.txt', 'w')
    ...
    file = open(results_path + '/xyz.txt', 'w')
    ...
    file1 = open(results_path + '/links.txt', 'w')
    file2 = open(results_path + '/links_distance.txt', 'w')
    set_check = set()
    for sat in mylinks:
